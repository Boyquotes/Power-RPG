...
func 产生地图():
	var walker = Walker.new(Vector2(949, 1642), 边界们)
	var map = walker.walk(500)
	walker.queue_free()
	for location in map:
		tilemap.set_cellv(location, -1)
	print("边界们:" + str(边界们.position))
	print("边界们:" + str(边界们.end))
	tilemap.update_bitmask_region(边界们.position, 边界们.end)
