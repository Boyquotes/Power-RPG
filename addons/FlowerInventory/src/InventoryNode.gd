...
export(int) var affix_front
export(int) var affix_end
export(int) var effect_front
export(int) var effect_end
...
func _instance_node(id:int, num:int, type:String):
	...
	random.randomize()
	# 随机id
	_add_node.item_affix = random.randi_range(affix_front, affix_end)
	
	# ==== 随机特殊效果（下面的） ====
	random.randomize()
	# 随机id
	_add_node.item_effect = random.randi_range(effect_front, effect_end)
...
func del_item(id:int, num:int, type:String) -> void:
	...
	for i in del_array:
		if now_item_num == 0:
			inventory_base.remove_child(get_child_i(del_array[i]))
		else:
